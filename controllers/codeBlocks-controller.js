...
const getCodeBlocksByUserId = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Fetching request failed. Please check input.",
      500
    );
    return next(error);
  }
...
const updateCodeBlock = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Something went wrong, could not update CodeBlock.",
      500
    );
    return next(error);
  }
...
const createCodeBlock = async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new HttpError("Could not find user for provided creator.", 404)
    );
  }
  ...
  res
    .status(201)
    .json({ codeBlock: createdCodeBlock.toObject({ getters: true }) });
};
